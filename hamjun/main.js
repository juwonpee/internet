...
function calculate(temp,wind,dust) {
    ...
    if(temp<7){tip = tip+'추운 날: 나이가 어리거나 많은 강아지, 소형견, 치와와와 같은 소형견은 체온조절 능력이 떨어져 따뜻한 옷을 입혀줘야 합니다. 집 안에서 기온이 낮은 베란다와 같은 곳에서 온도적응을 하고 나가시고, 준비운동으로 관절을 충분히 풀어주어야 합니다.';}
    if(dust>18){tip = tip+'미세먼지가 많은 날: 미세먼지가 많은 날에는 산책활동을 삼가는 것이 좋지만, 산책을 하게 된다면 물을 많이 섭취하는 것이 도움이 됩니다. 산책 후에 목욕을 하거나 반려동물용 물티슈와 빗으로 먼지를 털어주는 것이 필요합니다.';}
    if(wind.match(/\d+/g)[0]>2){tip = tip+'바람이 센 날: 바람이 불어 자신의 털이 날리는 것이 싫을 수 있고, 귀로 바람이 들어가 불편해할 수 있습니다. 또 바람소리 등 평소와 다름으로 인해 불안해할 수 있기 때문에 가급적 짧은 시간만 산책을 시켜주세요..';}
    alert(tip);
